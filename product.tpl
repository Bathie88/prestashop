...
<div itemscope itemtype="https://schema.org/Product">
	...
	<div class="primary_block row">
		...
		<div class="pb-left-column col-xs-12 col-sm-4 col-md-8">
			...
			<div id="image-block" class="clearfix">
				...
				{if $have_image}
					...
					{include file="$tpl_dir./SlideImagesProduct.tpl" images=$images}<!-- ajoutée-->
						...
						{else}
							...
							{if !$content_only}
								...
								<span class="span_link no-print"><i class="icon-eye-open" aria-hidden="true"></i></span>
							{/if}	
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
			...
			<div class="col-xs-12 col-sm-10"><h1 itemprop="name">{$product->name|escape:'html':'UTF-8'}</h1></div>
            
           <div class="col-xs-12 col-sm-2"> {if isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS}{$HOOK_PRODUCT_ACTIONS}{/if}</div>
           ...
           <div class="clearfix"></div>
			...
			<p id="product_references"{if empty($product->reference) || !$product->reference} style="display: none;"{/if}>
				<label>{l s='Reference:'} </label>
				<!--<span class="editables" itemprop="sku"{if !empty($product->reference) && $product->reference} content="{$product->reference}"{/if}>{if !isset($groups)}{$product->reference|escape:'html':'UTF-8'}{/if}</span>-->
			...
			<div class="addthis_inline_share_toolbox" id="productFP"></div>
		...
		<!-- ici se trouvait le bloc buy_block -->
...
{include file="$tpl_dir./blockCustomizerTest.tpl"} <!-- modif du 21-06-2017 -->
			...
			<section class="page-product-box customizer">
				...
				<form method="post" action="{$customizationFormTarget}" enctype="multipart/form-data" id="customizationForm" class="clearfix">
					...
					<p id="customizedDatas">
						...
						<button class="exclusive" name="saveCustomization">
							<span>{l s='Save'}</span>
						</button>
...
{/foreach}-->
       ...
       <div class="col-xs-12 col-sm-5 dimension">
       
       
       
       <!--<section class="page-product-box">
				<h3 class="page-product-heading">{l s='CARACTÉRISTIQUE ÉNERGÉTIQUE'}</h3>
                
                <img class="img-responsive" src="{$img_dir}pictogram.jpg" alt="">               
...
{/foreach}
       ...
       <section class="page-product-box transports">
				<h3 class="page-product-heading">{l s='LES TRANSPORTS'}</h3> 
                
              <ul>  
              <li><span>{l s='Métro :'}</span> {l s=' Station 01, Station 02, Station 03'}</li>
<li><span>{l s='Bus :'}</span> {l s='62, 96, 95'}</li>
<li><span>{l s='RER :'}</span> {l s='B'}</li>
</ul>
                  </section>
...
<div class="clearfix"></div>
		...
		{if isset($accessories) && $accessories}
			<!--Accessories -->
			<section class="page-product-box accessoires_global">
				<h3 class="page-product-heading">{l s='Accessories'}</h3>
                <h4 class="sub_title">{l s='En exclusivité pour vous'}</h4>
				<div class="block products_block accessories-block clearfix">
					<div class="block_content">
                    
                    
                    
                    {include file="$tpl_dir./product-list.tpl" products=$accessories class='bxslider' id='bxslider'}
...
<!--<script type="text/javascript">
	...
	$(document).ready(function(){		
		...
		//var hauteur = $(".customizer").height();
		var hauteur = $("#short_description_block").height();
		var hauteur_prix = 900;//bloc contenant qty et prix
		var hauteur_button = 980;//bouton ajouter au panier
		var hauteur_customizer = $(".customizer").height();//hauteur bloc personnalisation

		var height_1 = hauteur+hauteur_prix;
		var height_2 = hauteur+hauteur_button;
		var file = $("div").hasClass("customizableProductsFile");//test si les champs upload file sont actifs
		var ecran = $(window).width();
		var custo = $('#customizationForm').length;//tester si formulaire customization est présente
		var li = $('ul#text_fields > *').length;//nombre de champ textarea


		//alert(hauteur_customizer);

	//alert(custo);
	if(custo == 1){
		//alert(custo);
		if (file) {
			$(".Buttons").css("top", +height_1+"px");
			$(".Buttons").css("position", "absolute");
			$(".box-info-product .box-cart-bottom").css("top", +height_2+"px");
			$(".box-info-product .box-cart-bottom").css("position", "absolute");
		}

		//cas ou on a les champs upload
		if (file && ecran < 410) {
			//alert('ok');
			$(".Buttons").css("top", +height_1+"px");			
			$(".box-info-product .box-cart-bottom").css("top", +height_2+40+"px");	
			//$(".top").css("top", "260px");		
		}
		...
		if (!file && ecran < 410) {
			//alert('ok');
			$(".Buttons").css("top", +height_1+"px");			
			$(".box-info-product .box-cart-bottom").css("top", +h+"px");	
			//$(".top").css("top", "0px");		
		}
		//cas ou on pas les champ upload file
		if (!file) {
			$(".Buttons").css("top", +height_1-190+"px");
			$(".Buttons").css("position", "absolute");
			$(".box-info-product .box-cart-bottom").css("top", +height_2-190+"px");
			$(".box-info-product .box-cart-bottom").css("position", "absolute");
		}

		//cas ou on a les champ upload file
		if (file) {
			var hauteur_img = $(".customizationUploadBrowse img").height();//hauteur bloc image upload
			var h = hauteur_img * 2;
			var top = -400;
			//alert(hauteur_img);
			$(".Buttons").css("top", +height_1+h+"px");
			$(".Buttons").css("position", "absolute");
			$(".box-info-product .box-cart-bottom").css("top", +height_2+h+"px");
			$(".box-info-product .box-cart-bottom").css("position", "absolute");
			//$(".top").css("top", +top-h+"px");
		}
		//cas ou est sur une fiche produit de type Option
		var map = $('iframe').length;//voir si on a le blog desc longue,caracteristique,map google
		var caracteristique = $('.rte p img').length;//verifie si on a le bloc accessoire
		var accessoire = $('.accessoires_global').length;

		//alert(caracteristique);
		if(map == 0 && caracteristique == 0 && accessoire == 0){
			//alert('nous sommes sur une fp de type option');
			$(".blockproductscategory").css("margin-top", "600px");
			$(".accessoires_global").css("margin-top", "400px");
		}
		if(map == 0  && accessoire == 1){
			//alert('ok');
			$(".accessoires_global").css("margin-top", "220px");
			var hmoins = 1300;
			$(".Buttons").css("top", +height_1+h-hmoins+"px");
			$(".Buttons").css("position", "absolute");
			$(".box-info-product .box-cart-bottom").css("top", +height_2+h-hmoins+"px");
			$(".box-info-product .box-cart-bottom").css("position", "absolute");
		}
		//si on a plus de 3 textarea
		if(li > 3){
			//alert('test');
			var hplus = 200;
			$(".Buttons").css("top", +height_1+h+hplus+"px");
			$(".Buttons").css("position", "absolute");
			$(".box-info-product .box-cart-bottom").css("top", +height_2+h+hplus+"px");
			$(".box-info-product .box-cart-bottom").css("position", "absolute");
		}
		//si on a plus de 5 textarea
		if(li >= 5){
			//alert('test');
			...
			$(".box-info-product .box-cart-bottom").css("top", +height_2+h+hplus+"px");
			$(".box-info-product .box-cart-bottom").css("position", "absolute");
			$(".blockproductscategory").css("margin-top" , +percent+"%");
			//alert(h);
		}
		//sur les pages produit d'art
		var accessoire = $('.accessoires_global').length;
		var champ_file = $('.customizationUploadLine').length;
		//alert(champ_file);
		//alert(map);
		if(accessoire == 0 || champ_file == 1){
			//alert(h);
			var hplus = -350;
			$(".Buttons").css("top", +height_1+h+hplus+"px");
			$(".Buttons").css("position", "absolute");
			$(".box-info-product .box-cart-bottom").css("top", +height_2+h+hplus+"px");
			$(".box-info-product .box-cart-bottom").css("position", "absolute");
			//$(".blockproductscategory").css("margin-top" , "30%");
		}
		/*if(file >= 1 || file <= 3 && li > 3){
			alert(li);
			var hplus = -400;
			$(".Buttons").css("top", +height_1+h+hplus+"px");
			$(".Buttons").css("position", "absolute");
			$(".box-info-product .box-cart-bottom").css("top", +height_2+h+hplus+"px");
			$(".box-info-product .box-cart-bottom").css("position", "absolute");
			$(".accessoires_global").css("margin-top", "320px");
		}*/
